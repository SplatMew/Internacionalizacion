...
$sql = "SELECT * FROM intercambio_estudiantil_salida WHERE ID = ${_GET["id"]}";
...
<body>
	...
	<div class="wrapper d-flex align-items-stretch">
		...
		<div id="content" class="p-4 p-md-5 pt-5">
	      ...
	      navVar("Sistema de Internacionalización > Estudiantes > Salida > Consultar Intercambios > Actualizar Intercambio","../../public/assets/UABC_crop.png")
	    ?>
			...
			<div id="Page1">
				...
				<div class="container">
	          	...
	          	<div class="col-sm-4">
              	<div class="d-flex justify-content-center p-1">
                	<button type="submit" class="btn btn-outline-primary" style="display: none;" name="btn_aplicarCambios_intercambio_salida" id="btn_aplicarCambios1" onclick="return confirmarAplicarCambios()">Aplicar Cambios</button>
              	</div>
	          	</div>
              	...
              	<div class="d-flex justify-content-center p-1">
                	<button type="button" class="btn btn-danger" name="btn_eliminar" id="btn_eliminar" onclick="return confirmarBorrar()" <?php if (!$_SESSION["admin"]) echo "disabled"; ?>>Eliminar</button>
						...
						<div class="row mb-3">
							...
							<div class="col-sm-6">
								<input type="text" class="form-control border border-secondary" name="unidad_reseptora_nombre" id="unidad_reseptora_nombre" value="<?php echo $res["UR"]; ?>" disabled>
							</div>
						</div>
						...
						<div class="row mb-3">
							...
							<div class="col-sm-6">
								<input type="text" class="form-control border border-secondary" name="unidad_reseptora_pais" id="unidad_reseptora_pais" value="<?php echo $res["UR_PAIS"]; ?>" disabled>
							</div>
						</div>
						...
						<div class="row mb-3">
							...
							<div class="col-sm-6">
								<input type="text" class="form-control border border-secondary" name="unidad_reseptora_entidad" id="unidad_reseptora_entidad" value="<?php echo $res["UR_ENTIDAD"]; ?>" disabled>
							</div>
						</div>
						...
						<div class="row mb-3">
							...
							<div class="col-sm-6">
								<input type="text" class="form-control border border-secondary" name="unidad_reseptora_idioma" id="unidad_reseptora_idioma" value="<?php echo $res["UR_IDIOMA"]; ?>" disabled>
							</div>
						</div>
						...
						<div class="row mb-3 justify-content-md-center">
							<button type="submit" class="btn btn-outline-primary" style="display: none;" name="btn_aplicarCambios_intercambio_salida" id="btn_aplicarCambios2" onclick="return confirmarAplicarCambios()">Aplicar Cambios</button>
						</div>
					</form>
	...
	<script>
		...
		btn_activar_edicion.addEventListener("click", () => {
			...
			if (document.getElementById("campus_clave").disabled === true) {
		...
		function confirmarBorrar() {
			...
			if (confirm('¿Estás seguro que quieres eliminar el registro del intercambio del estudiante de matrícula ' + mat + '?')) {
				window.location.href = "../../php-partials/delete.php?tabla=intercambio_salida&id=" + id;
			} else {
				return false;
			}
		}
