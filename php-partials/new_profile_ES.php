<?php 
	...
	$estudiante_id = mysqli_real_escape_string($con, $_POST['matricula']);
	$correo = mysqli_real_escape_string($con, $_POST['correo']);
	$sql = "SELECT * FROM usuarios WHERE matricula=${estudiante_id} OR correo='${correo}'";
    ...
    if(!AddNewUserNOADMIN($_POST,$con,10)){
    	mysqli_close($con);
    ...
    $sql = "INSERT INTO perfil_estudiantes_salida (
    	ESTUDIANTE_ID, ESTUDIANTE, ESTUDIANTE_APELLIDO1, ESTUDIANTE_APELLIDO2,
    ...
    if (mysqli_query($con, $sql)) {
        mysqli_close($con);
        PantallaError("../public/assets/UABC_crop.png","OPERACIÓN EXITOSA","Su perfil de estudiante local de UABC fue creado exitosamente en el sistema de internacionalización, ahora puede acceder utilizando su correo y contraseña en la página de inicio.",0);
    } else {
    	mysqli_close($con);
